...
\subsection{数值算例与覆盖复核}

取真实源位置 $G=(0,0)$，在三个检测点分别加入 $+0.3^\circ$、$-0.6^\circ$ 和 $+0.2^\circ$ 的示向误差。半平面交得到含 6 个顶点的定位多边形，最远顶点对为 $(27.756,-23.083)$ 与 $(-14.648,1.119)$，直径为 $48.824$ 米。最小包围圆半径为 $24.475$ 米，而半直径为 $24.412$ 米，有两个顶点位于直径圆外，故该算例不能被最远点对对应的直径圆覆盖。图~\ref{fig:q1-numerical-example} 同时给出全局交会关系和局部覆盖检验。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/q1q2_word/image2.png}
  \caption{三次示向约束形成的定位区域及直径圆覆盖检验}
